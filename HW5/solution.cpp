...
std::vector<int> compress_row(const std::vector<int>& row) {
    ...
    copy_if(row.begin(), row.end(), back_inserter(compressed), [](int x){ return x != 0;}); // Compress
    while (compressed.size() < 4) compressed.push_back(0); // Pad with zeros
    return compressed;
}
...
std::vector<int> merge_row(std::vector<int> row) {
    for (int i = 0; i < 3; i++) {
        if (row[i] != 0 && row[i] == row[i+1]) {
            row[i] *= 2;
            row[i+1] = 0;
            i++; // skip next to prevent double merge
        }
    }
...
void print_board(const vector<vector<int>>& board) {
    // Print the score using compute_score(board)
    cout << "Score: " << compute_score(board) << endl;
    // Print the board in a 4x4 format
    // Use dots (.) for empty cells (value 0)
    // Use tabs (\t) to separate values for alignment
...
bool move_left(std::vector<std::vector<int>>& board) {
    ...
    //   3. Check if the row changed
        for (int r = 0; r < 4; r++) {
            ...
            vector<int> compressed = compress_row(original_row);
...
bool move_up(std::vector<std::vector<int>>& board) {
    ...
    // TODO: Extract column, compress, merge, write back
        for (int c = 0; c < 4; c++) {
            ...
            vector<int> compressed = compress_row(column);
            vector<int> merged = merge_row(compressed);
            for (int r = 0; r < 4; r++) {
